...
exports.getProducts = async (req, res) => {
  try {
    ...
    const response = {
      ...
      nextPage:
        pageInt < Math.ceil(totalProducts / limitInt) ? pageInt + 1 : null,
      page: pageInt,
...
exports.addProduct = async (req, res) => {
  ...
  validateProduct(productAdd, res);
...
exports.deleteProduct = async (req, res) => {
  ...
  try {
    ...
    if (deletionResponse.deletedCount) {
      ...
      res.json({
        message:
          "Producto eliminado exitosamente y correos enviados a usuarios premium",
      });
    } else {
      res.status(404).json({ error: "Producto no encontrado" });
    }
